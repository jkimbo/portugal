 ...
 *
 * Modifications to original source made by Rowan Manning
 * https://github.com/rowanmanning/timthumb
 *
...
// preset image sizes (used if the USE_PRESET_SIZES configuration is TRUE)
$preset_sizes = array(
	'small_square' => array(100, 100),
	'medium_square' => array(300, 300),
	'large_square' => array(800, 800),
	'small_landscape' => array(480, 360),
	'medium_landscape' => array(640, 480),
	'large_landscape' => array(800, 600),
	'small_portrait' => array(360, 480),
	'medium_portrait' => array(480, 640),
	'large_portrait' => array(600, 800),
);
...
// get image type early (before we check the cache)
$type = strtolower(get_request ('t', null));
$type = ($type == 'jpg' || $type == 'png' ? $type : 'jpg');
...
// use preset image sizes if required
if (USE_PRESET_SIZES) {

	// reset the new width/height – we won't be using those
	$new_width = $new_height = 0;

	// get the preset width/height
	if (isset($preset_sizes[$preset_size]) && isset($preset_sizes[$preset_size][0]) && isset($preset_sizes[$preset_size][1])) {
		$new_width = (int) $preset_sizes[$preset_size][0];
		$new_height = (int) $preset_sizes[$preset_size][1];
	}
...
function show_image ($mime_type, $image_resized) {
    ...
    $cache_file = get_cache_file ($type);
